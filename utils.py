...
def mkdir_if_needed(dir_name):
    mypath = Path(dir_name)
    mypath.mkdir(exist_ok=True, parents=True)
...
def norm_grad(grad_list):
    client_grads = grad_list[0].cpu().numpy()

    for i in range(1, len(grad_list)):
        client_grads = np.append(
            client_grads, grad_list[i].cpu().numpy())
...
class HfArgumentParser(ArgumentParser):
    """
    HfArgumentParser是一个继承自argparse.ArgumentParser的子类,它在数据类上使用类型提示来生成命令行参数解析。
    """
